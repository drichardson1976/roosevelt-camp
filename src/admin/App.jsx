    ...
    export function RooseveltCamp() {
      ...
      useEffect(() => {
        const load = async () => {
          ...
          try {
            // Phase 1: Core tables for Dashboard, Counselors, Parents, Campers, Registrations (7 tables)
            const [cd, cs, rg, pr, ad, cp, cpl] = await Promise.all([
          ...
          console.log('⏱️ [ADMIN STEP 4] Phase 1 loaded (7 tables): ' + (performance.now() - window.__t0).toFixed(0) + 'ms (DB took ' + (performance.now() - dbStart).toFixed(0) + 'ms)');
          setLoading(false);

          // Phase 2: Remaining 13 tables loaded in background
          const phase2Start = performance.now();
          try {
            const [cu, av, ch, as2, fp, sp, ec, op, acr, pcr, bs, csch, gr] = await Promise.all([
          ...
          console.log('⏱️ [ADMIN STEP 5] Phase 2 loaded (13 tables): ' + (performance.now() - window.__t0).toFixed(0) + 'ms (DB took ' + (performance.now() - phase2Start).toFixed(0) + 'ms) — all data ready');
        };
        load();
      }, []);
